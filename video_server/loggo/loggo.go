...
func New(logPath string, prefix string, flag int,  level LogLevel) *Loggo {
	//获取日期
	...
	loggo := &Loggo{log: logger, level: level, logPath: logPath, currentLogFile: logFile}

	//开启日期检测并替换log文件
	go loggo.CheckDateAndNewFile()
...
func (loggo *Loggo) Debug(e interface{})  {
...
func (loggo *Loggo) Info(e interface{})  {
	if loggo.level <= Info {
		var s string
		//判断最常用的string和error，免得每次都fmt.Sprint(e)反射得到类型，反射时间消耗多
		switch e := e.(type) {
...
func (loggo *Loggo) Warn(e interface{})  {
	if loggo.level <= Warn {
		var s string
		//判断最常用的string和error，免得每次都fmt.Sprint(e)反射得到类型，反射时间消耗多
		switch e := e.(type) {
...
func (loggo *Loggo) Error(e interface{})  {
...
func (loggo *Loggo) CheckDateAndNewFile()  {
	for  {
